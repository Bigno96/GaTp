...
class PerformanceRecorder:
    ...
    def evaluate_performance(self,
        ...
        task_percentage = 1 - ((len(self.simulator.active_task_list) + agent_active_tasks)
                               / self.simulator.task_num)
        # count collisions
        collisions, _, _ = count_collision(agent_schedule=self.simulator.agent_schedule)
...
def count_collision(agent_schedule: Dict[int, Deque[Tuple[int, int, int]]]
    ...
    :return: number of collision detected,
             ...
             list of collisions at each timestep
    """
    ...
    time_view = list(map(lambda v: drop_ts(v),  # remove timesteps
    ...
    # remove duplicates from collision time list
    return sum(coll_list), list(set(collision_time_list)), coll_list


def drop_ts(input_list: List[Tuple[int, int, int]]
            ) -> List[Tuple[int, int]]:
    """
